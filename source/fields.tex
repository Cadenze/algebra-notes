...
\begin{remark}
    ...
    \(\mfk{p}\) a prime ideal, and \(\mfk{m}\) a maximal ideal,
    then \(R/\mfk{p}\) \hyperref[thm:prime-quotient-domain]{is an integral domain}
    and \(R/\mfk{m}\) \hyperref[thm:maximal-quotient-field]{is a field}.
    But when \(R\) is a field,
...
\section{Algebraic Extensions}\label{sec:algebraic-extensions}
...
\begin{proof}
    ...
    Lastly we prove that (ii) implies (i).
...
\end{theorem}
\begin{proof}
    Let \(\mfk{m} = (f(x)) \subseteq F[x]\) be a maximal ideal,
    since \(f(x)\) is irreducible by Theorem~\ref{thm:ideal-divisibility}.
    Then we have a field \(K = F[x]/\mfk{m} \supseteq F\)
    ...
    and \(\func{\pi}{F[x]}{K}\) is the quotient by \(\mfk{m}\),
    both of them ring homomorphisms.
    We can see that \(\pi\circ\iota\) is a field homomorphism,
    since elements in \(F\) within \(F[x]\) will not get collapsed by \(\mfk{m}\)
...
\begin{definition}
    Suppose \(K/F\) is an extension.
    An automorphism of \(K\) is a field automorphism \(\sigma \in \Aut(K)\)
    that is bijective;
    an \(F\)-automorphism of \(K\) is a field automorphism \(\sigma \in \Aut_F(K)\),
...
% \end{definition}
\begin{lemma}\label{lem:splitting-field-properties}
    Suppose \(F\) is a field.
...
\end{lemma}
\begin{proof}
    (i) implies (ii) is the contrapositive of Corollary~\ref{cor:algebraic-finite-extension}.

    (ii) implies (iii) because if there exists another \(F\)-algebraic element \(a \in K-F\),
    then there is a finite extension \(F(a)/F\).

    (iii) implies (iv) since if \(f(x) \in F[x]\) and \(a\) is a root,
    then \(a\) is algebraic over \(F\), and must be in \(F\).
    Induct on order of \(f\).

    (iv) implies (v) trivially true.

    (v) implies (iv) simply by induction on degree of \(f\).

    (iv) implies (i) since if there are other algebraic extensions,
...
\begin{definition}
    ...
    denoted \(K = F^\text{alg} = \overline{F}\).
\end{definition}
\begin{theorem}[Existence of Algebraic Closures]\label{thm:existence-alg-closure}
    Suppose \(F\) is a field.
...
\begin{proof}
    ...
    We now wish to show that the ideal \(\mfk{a}\) generated by all \(f(X_f)\)
    is not the unit ideal \((1) = F[S]\).
    Suppose, by way of contradiction, that \(\mfk{a} = (1) = F[S]\).
    Then there exists a polynomial
    \begin{equation*}
        g_1 f_1(X_{f_1}) + \cdots + g_n f_n(X_{f_n}) = 1
    \end{equation*}
    where \(g_i \in F[S]\).
    If we denote \(X_i = X_{f_i}\),
    since each \(g_i\) only involves finitely many \(X_j\),
    we can write this as
    \begin{equation*}
        \sum_{i=1}^n g_i(X_1,\hdots,X_N) f_i(X_i) = 1
    \end{equation*}
    for some \(N \geq n\).
    Each \(f_i\) has some root \(\alpha_i\),
    so allow us to substitute \(\alpha_i = 1\) for the first \(n\) variables,
    and \(\alpha_i = 0\) for the rest.
    Then clearly the linear combination evaluates to 0, which is a contradiction.

    Allow \(\mfk{m}\) to be the maximal ideal containing \(\mfk{a}\)
    (which exists due to \hyperref[thm:krull]{Krull's theorem}).
    Then there exists a quotient map \(\func{\pi}{F[S]}{F[S]/\mfk{m} = E_1}\),
    where \(E_1\) is a field (Theorem~\ref{thm:maximal-quotient-field}).
    Clearly \(E_1\) contains the roots to every polynomial in \(F[x]\),
    because \(\mfk{a} \subseteq \mfk{m}\), and hence \(X_f\) is a root of \(f\).

    We can now replace \(F\) with \(E_1\),
    and using the same procedure, construct a field \(E_2\) that contains all roots to \(E_1[x]\).
    This procedure can be continued indefinitely, and get fields \(E_n\) for every \(n\).
    We then verify that \(K = bigcup_{n=1}^\infty E_n\) is a field.
    Obviously, given finitely many elements \(K\), they must both belong in some \(E_n\),
    and we can perform our operations as desired within that field.

    Lastly, we see that any polynomial in \(K\) has finitely many coefficients,
    so they belong in some \(E_n\).
    Obviously the roots to that polynomial now belong in \(E_{n+1} \subseteq K\) as desired.
\end{proof}
\begin{corollary}
    Suppose \(F\) is a field,
...
\end{corollary}
\begin{proof}
    By the \hyperref[thm:existence-alg-closure]{theorem above},
    consider the algebraic closed field \(K/F\).
    Let \(F^\text{alg}\) be the union of all algebraic extensions of \(F\),
    which is a subextension of \(K\).
    Then \(F^\text{alg}\) is algebraic over \(F\) as desired.

    % Now consider any algebraic element \(\alpha \in E\) over \(F^\text{alg}\).
    Now consider any \(f \in F^\text{alg}[x]\), and some root \(\alpha \in E\).
    Because \(\alpha\) is algebraic over \(F^\text{alg}\),
    it is then algebraic over \(F\).
    Then it must be contained in algebraic extension of \(F\), so it is in \(F^\text{alg}\).
    Hence \(F^\text{alg}\) is algebraically closed.
\end{proof}

\begin{proposition}\label{prop:bijection-extension-roots}
    Suppose \(\func{\sigma}{F}{L}\) is a field homomorphism,
    where \(L = \overline{L}\) is algebraically closed.
    Let \(\alpha\) be algebraic over \(F\),
    \(p(x) = \min_F(\alpha) = \sum_{i=0}^n a_i x^i\) a monic minimal polynomial,
    and \(\sigma(p)(x) = \sum_{i=0}^n \sigma(a_i)x^i\) its image.
    Then given a homomorphism \(\func{\tau}{F(\alpha)}{L}\)
    that extends of \(\sigma\) (\(\tau\vert_F = \sigma\)),
...
\begin{proof}
    ...
    Since \(p(\alpha) = 0\),
    we have \(\tau(p(\alpha)) = \sigma(p)(\tau(\alpha)) = \sum_{i=0}^n \sigma(a_i){\tau(\alpha)}^i = 0\).
    Hence \(\tau(\alpha)\) is indeed a root of \(\sigma(p)\),
    guaranteeing us a well-defined map \(\tau \mapsto \tau(\alpha)\).

    We show injectivity first.
    Suppose \(\beta \in L\), such that \(\sigma(p)(\beta) = 0\).
    Observe that for any \(\func{\tau}{F(\alpha)}{L}\),
    it is wholly defined by its image on \(F\) and on \(\alpha\).
    So given any \(\tau(\alpha) = \beta\),
    and that \(\tau\) restricts to \(\eval{\tau}_F = \sigma\),
    we have wholly defined \(\tau\), and there is a unique \(\tau\) that maps to \(\beta\).
\end{proof}
...
\end{theorem}
\begin{proof}
    We first consider the simplest case, where \(p\) is linear.
    Then this is obvious, as it already splits, so \(\sigma = \phi\).

    We now proceed by strong induction, assuming that this holds for all polynomials of degree less than \(n\),
    where \(n\) is the degree of our irreducible polynomial \(p\).
    Then \(\phi(p(x))\) is also irreducible in \(F\), of degree \(n\).
    Observe that \(K\) and \(L\) contains all roots of \(p\) and \(\phi(p)\) respectively,
    so take a root \(a\) of \(p\) and \(b\) of \(\phi(p)\).
    By the \hyperref[prop:bijection-extension-roots]{proposition above},
    we have the commutative diagram
    \begin{equation*}
        \begin{tikzcd}
            K \arrow{r}{\sigma} & L \\
            E(a) \arrow{r}{\phi^\ast} \arrow[dash]{u} & F(b) \arrow[dash]{u} \\
            E \arrow[dash]{u} \arrow{r}{\phi} & F \arrow[dash]{u}
        \end{tikzcd}
    \end{equation*}
    where the top part is guaranteed by induction, since it is of degree \(n-1\).

    We note that the same proof applies when \(p\) is not irreducible,
    by simply choosing the irreducible part before applying induction.

    The special case is merely when \(E = F\).
\end{proof}
...
\begin{proposition}\label{prop:discriminant-invariant}
...
\begin{lemma}
    ...
    \(f(x)\) is separable if and only if \(\gcd(f,f') = 1\) are coprime.
\end{lemma}
\begin{proof}
    ...
    Without loss of generality we may assume \(\gcd(g,h) = 1\) are also coprime,
    ...
    Hence \(\gcd(f,f') = 1\) are coprime.

    Now suppose \(\gcd(f,f') = 1\).
...
\end{definition}
\begin{theorem}\label{thm:homomorphism-set-bijection}
    Let \(\func{\sigma}{F}{\img\sigma \subseteq L}\) be an embedding of fields,
    where \(L\) is algebraically closed,
    and similarly, let \(\func{\tau}{F}{\img\tau \subseteq L'}\) be an embedding of fields,
    where \(L'\) is algebraically closed.
    Define \(\lambda\) to be the extension of \(\func{\tau\circ\sigma^{-1}}{\img\sigma}{\img\tau}\)
    to the isomorphism \(\func{\lambda}{L}{L'}\).
    The map \(\func{\psi}{S_\sigma}{S_\tau}\)
    that sends \(\sigma^\ast\) to \(\lambda\circ\sigma^\ast\) is a bijection.
\end{theorem}
\begin{proof}
    Consider any extension \(\sigma^\ast \in S_\sigma\).
    For any \(x \in F\),
    \(\lambda\circ\sigma^\ast(x) = \lambda\circ\sigma(x) = \tau\circ\sigma^{-1}(\sigma(x)) = \tau(x)\).
    Hence \(\lambda\circ\sigma^\ast\) is an extension of \(\tau\),
    meaning \(\lambda\circ\sigma^\ast \in S_\tau\).
    Since \(\lambda\) is an isomorphism, this implication also holds for \(\lambda^{-1}\),
    which proves the other direction.
\end{proof}
\begin{corollary}
    If \(E/F\) is an algebraic extension,
    and \(\func{\sigma}{F}{L}\) embeds \(F\) into an algebraically closed field \(L\),
    then the number of extensions of \(\sigma\) is independent of the choice of \(\sigma\),
    but only on the extension \(E/F\).
\end{corollary}
\begin{proof}
    The bijection in the \hyperref[thm:homomorphism-set-bijection]{theorem above}
    gives us the equality of the number of extensions.
\end{proof}
...
\begin{proposition}\label{prop:algebraic-extension-hom}
    ...
    and \(L\) is the algebraic closure of \(\img\sigma\),
    then \(\tau\) is an isomorphism.
\end{proposition}
\begin{proof}
    Consider the set of tuples \(\Sigma = \{(K,\tau): F \subseteq K \subseteq E,\; \func{\tau}{K}{L}\;\text{extending}\;\sigma\}\)
    of intermediate fields \(K\) and extensions of \(\sigma\) to \(K\).
    The trivial extension \((F,\sigma)\) is in this set, so \(\Sigma \neq \emptyset\).
    We can endow \(\Sigma\) with a partial order \(\preceq\),
    where \((K,\tau) \preceq (K',\tau')\) when \(K \subseteq K'\) and \(\eval{\tau'}_K = \tau\).
    Now consider a chain \({(K_i,\tau_i)}_{i=1}^\infty \subset \Sigma\).
    \(K = \bigcup_{i=1}^\infty K_i\) is a subfield of \(E\) (same proof as rings suffice),
    and we can define \(\func{\tau}{K}{L}\) by sending \(x \mapsto \tau_i(x)\) for \(x \in K_i\).
    Zorn's lemma then implies we have a maximal element \((K,\tau) \in \Sigma\).

    It now suffices to show that \(K = E\).
    Suppose there exists \(\alpha \in K - E\).
    Since \(\alpha\) is algebraic over \(K\),
    we can then extend \(\func{\tau}{K}{L}\) to \(\func{\tau^\ast}{K(\alpha)}{L}\),
    contradicting maximality.
    We have that \(K = E\).

    Now, if \(E\) is an algebraic closure of \(F\) and \(L\) is an algebraic closure of \(\img\sigma\),
    since \(\img\tau\) is algebraically closed and \(L/\img\tau\) is algebraic,
    we have equality \(L = \img\tau\) as desired.
\end{proof}
...
\end{proposition}
\begin{proof}
    Let \(E = F(\alpha_1,\hdots,\alpha_k)\) for some \(k\) field generators.
    If there is only 1 generator,
    the number of ways to extend an embedding is at most the degree of \(\min_F(\alpha)\),
    which is obviously equal to \([F(\alpha):F]\).

    We now perform weak induction on the number of generators.
    Define \(E_i = F(\alpha_1,\hdots,\alpha_i)\),
    so that \(E_{i+1} = E_i(\alpha_{i+1})\) and \(E = E_k\).
    Then by induction, there are at most \([E_{k-1}:F]\) different embeddings
    extending \(\sigma\) to \(E_{k-1}\),
    and each of those embeddings have at most
    \(\deg\min_F(\alpha_k) = [E_{k-1}(\alpha_k):E_{k-1}] = [E:E_{k-1}]\) extensions to \(E\).
    Hence in total, there are at most \([E:E_{k-1}][E_{k-1}:F] = [E:F]\) extensions of \(\sigma\).
\end{proof}
...
\end{theorem}
\begin{proof}
    Let \(E = F(\alpha_1,\hdots,\alpha_k)\) and \(E_i = F(\alpha_i,\hdots,\alpha_i)\) just as before.
    We already have that the number of such extensions is at most \([E:F]\) (Proposition~\ref{prop:algebraic-extension-hom-count-bound}),
    so for the reverse direction, it suffices to prove that it is at least \([E:F]\).
    But combining the inductive argument from Proposition~\ref{prop:algebraic-extension-hom-count-bound}
    and the bijection from Propostion~\ref{prop:algebraic-extension-hom},
    we have equality.

    For the other direction, suppose \(E/F\) is not separable.
    Then there is some inseparable element \(\beta \in E\).
    Consider the extension \(F(\beta)/F\).
    Because there must be at least a double root in \(\min_F(\beta)\),
    there are strictly fewer than \([F(\beta):F]\) extensions of \(\sigma\).
    Hence the number of extensions to \(E\) must be fewer than \([E:F(\beta)][F(\beta):F] = [E:F]\) as desired.
\end{proof}
...
\end{proposition}
\begin{proof}
    Suppose \(L/F\) is separable, which means every element of \(L\) is separable over \(F\).
    Then because \(E \subseteq L\), every element of \(E\) is separable over \(F\),
    and hence \(E/F\) is separable.

    Now consider any \(\alpha \in L\), and its minimal polynomial \(\min_F(\alpha) \in F[x]\).
    By definition of minimal polynomials, we know that \(\min_E(\alpha) \mid \min_F(\alpha)\).
    Since \(\alpha\) is separable over \(F\), the roots of \(\min_F(\alpha)\) are distinct,
    so the roots of \(\min_E(\alpha)\) are also distinct, giving us separability.
\end{proof}
...
\end{theorem}
\begin{proof}
    Suppose \(\func{\sigma}{F}{\overline{F}}\) is an embedding.
    Because \(E/F\) is separable, there are \([E:F]\) distinct embeddings \(\sigma^\ast\)
    from \(E\) extending \(\sigma\);
    similarly, because \(L/E\) is separable, there are \([L:E]\) distinct embeddings from \(L\)
    extending each \(\sigma^\ast\).
    Hence, there are a total of \([L:E][E:F] = [L:F]\) distinct embeddings extending \(\sigma\).
    By Theorem~\ref{thm:algebraic-extension-hom-count},
    \(L/F\) must be separable.
\end{proof}
...
\end{definition}
% \begin{lemma}\label{lem:separable-degree-multiplicative}
%     Suppose \(F \subseteq E \subseteq L\) is a tower of field extensions.
%     Then \({[L:F]}_s = {[L:E]}_s{[E:F]}_s\).
% \end{lemma}
% \begin{proof}
%     Given \(\sigma\) from \(F\),
%     extending via \(E\) to \(L\) is exactly the same as extending directly to \(L\).
%     Hence counting is multiplicative.
% \end{proof}
...
\end{proposition}
\begin{proof}
    Corollary of Proposition~\ref{prop:separable-degree-polynomial}.
\end{proof}
\begin{corollary}
...
\begin{proof}
    Given \(\sigma\) from \(F\),
    extending via \(E\) to \(L\) is exactly the same as extending directly to \(L\).
    Hence counting separable degree is multiplicative.

    By definition, because both separable degree and degree of extension are both multiplicative,
    we get the inseparable degree for free.
\end{proof}
...
% \end{proof}
% \begin{remark}
%     The standard proof for the primitive element theorem uses the full framework of Galois theory,
%     so we provide a more complicated proof with less prerequisite here (as stated in van der Waerden).
%     We shall prove this theorem with Galois theory in Theorem~\ref{thm:primitive-element-2}
% \end{remark}
...
\end{proposition}
\begin{proof}
    Because polynomials in \(F[x^{p^r}]\) must have degree at least \(p^r\),
    \(f \in F[x^{p^r}]\) only for finitely many \(r\).
    Allow us to consider the maximal \(m\) such that \(f \in F[x^{p^m}]\),
    and in particular, let \(f(x) = g(x^{p^m})\).
    By maximality of \(m\), \(g \notin F[x^p]\), giving us separability.
    Moreover, \(g(x)\) is irreducible, because if it is, i.e.\ \(g(x) = h(x)k(x)\),
    then \(f(x) = h(x^{p^m})k(x^{p^m})\) is reducible, which is a contradiction.
\end{proof}
\begin{definition}
    We call the degree of \(g\) the inseparable degree of \(f(x)\),
    and \(p^m\) the separable degree of \(f(x)\).
\end{definition}
\begin{proposition}\label{prop:separable-degree-polynomial}
...
\end{proposition}
\begin{proof}
    Obviously, \(\deg f(x) = [E:F]\),
    so it suffices to show that \({[E:F]}_s\) (the number of extensions of an embedding)
    is the separable degree of \(f(x)\) (\(\deg g(x)\)).
    But this is obvious, because \(g(x)\) is the version of \(f(x)\)
    but with all the repeated roots deleted, so we are done.
\end{proof}

\begin{proposition}\label{prop:irreducible-multiplicity}
    Suppose \(F\) is a field.
    Then all roots of an irreducible polynomial \(f(x) \in F[x]\) have the same multiplicity.
    In particular,
...
\end{lemma}
\begin{proof}
    If \(\alpha^{p^n} = a \in F\), then we are immediately done because it is the root of \(x^{p^n} - a\).
    Then because \(F\) has characteristic \(p\), this factors into \({(x-\alpha)}^{p^n}\),
    and because we know that \(\min_F(\alpha)\) divides this polynomial,
    the minimal polynomial must also be of this form.

    For the other direction, if \(\alpha\) is purely inseparable, let \(f(x) = \min_F(\alpha)\).
    By Proposition~\ref{prop:separable-power-polynomial},
    there exists some \(g\) such that \(f(x) = g(x^{p^m})\).
    If \(g\) factors over a splitting field,
    then we know that \(f(x) = \prod_{i=1}^r (x^{p^m} - b_i)\) over \(r\) distinct roots;
    but sicne \(f\) is purely inseparable, \(r = 1\),
    so \(f(x) = x^{p^m} - a\) for a root \(a\) as desired.
\end{proof}
...
\end{theorem}
\begin{proof}
    From the \hyperref[lem:purely-inseparable-power]{lemma above},
    we know that \(\min_F(\alpha)\) has only one distinct root due to inseparability,
    and separability implies that it has no repeated roots,
    so it must be linear, and in \(F\) as a result.
\end{proof}
\begin{theorem}
...
\end{theorem}
\begin{proof}
    Since \(K\) is purely inseparable, for all \(\alpha \in K\),
    \(\min_F(\alpha)\) splits over \(K\) as it is the only root;
    and as we have established, \(\alpha \in K\), so \(K\) is normal.

    If \(K\) is also finite, and \(F\) is characteristic \(p\),
    then \(K\) is finitely generated, with \(K = F(\alpha_1,\hdots,\alpha_n)\).
    By multiplicativity of degree, without any loss of generality, let \(K = F(\alpha)\).
    But by Lemma~\ref{lem:purely-inseparable-power} this is a power of \(p\).
\end{proof}
\begin{theorem}
    Suppose \(X\) a set, and \(K = F(X)\),
    with all \(\alpha \in X\) purely inseparable.
    Then \(K\) is purely inseparable.
\end{theorem}
\begin{proof}
    We prove that every \(a \in K\) is purely inseparable.
    Let \(a \in F(\alpha_1,\hdots,a_n)\) for some finitely many \(a_i \in X\).
    Choose \(m\) such that \(\alpha_i^{p^m} \in F\) for all \(i\).
    Because \(a\) can be written as a polynomial of \(\alpha_i\),
    we also have \(a^{p^m} \in F\).
    Hence \(\min_F(a) \mid {(x-a)}^{p^m}\), which means it only has one distinct root,
    giving us that \(a\) is purely inseparable.
\end{proof}
\begin{theorem}
...
\end{theorem}
\begin{proof}
    Suppose \(K/F\) is purely inseparable.
    For any \(a \in K\), there exists some \(m\) such that \(a^{p^m} \in F \subseteq L\),
    so \(K/L\) is purely inseparable.
    Moreover, since any \(a \in L \subseteq K\), \(L/F\) is also purely inseparable.

    Conversely, suppose \(L/F\) and \(K/L\) are both purely inseparable.
    Let \(a \in K\), and \(a^{p^m} \in L\) for some \(m\),
    and \({(a^{p^m})}^{p^r} = a^{p^{m+r}} \in F\) for some \(r\).
    Hence \(K/F\) is purely inseparable.
\end{proof}
...
\end{definition}
\begin{proposition}\label{prop:perfect-frobenius-bijective}
    Suppose \(F\) is a field with characteristic \(p\).
...
\begin{proof}
    Suppose that \(F^p = F\).
    We want to show that every algebraic extension is separable,
    but since separable extensions are stackable (Theorem~\ref{thm:separable-transitive}),
    we can show that every possible element in its algebraic closure is separable,
    i.e.\ every irreducible polynomial it \(F[x]\) is separable.
    Suppose, by way of contradiction, that \(f \in F[x]\) is irreducible and inseparable.
    Then since each root has multiplicity \(p^n\) (Proposition~\ref{prop:irreducible-multiplicity}),
    \(f(x)\) is a polynomial in \(x^p\),
    \begin{equation*}
        f(x) = a_n x^{pn} + a_{n-1} x^{p(n-1)} + \cdots + a_1 x^p + a_0 \in F[x^p]
    \end{equation*}
    so \(f' = 0\) after taking the derivative term-by-term.
    If \(F^p = F\), then we know that \(a_i = b_i^p\) for each \(i\),
    which implies \(f(x) = {(g(x))}^p\), which contradicts irreducibility.

    Now suppose \(F^p \neq F\), and we wish to prove that \(F\) is imperfect.
    Then there is some \(a \in F - F^p\) that does not have a \(p\)th root in \(F\),
    which means that \(x^p - a \in F[x]\) is irreducible, and has one root only.
    Consider this over a splitting field,
    such that \(x^p - a = {(x-b)}^p\) is irreducible and inseparable.
    Then \(F(b)/F\) is an inseparable extensions, which means \(F\) cannot be perfect.
\end{proof}
\begin{corollary}
    Finite fields are perfect.
\end{corollary}
\begin{proof}
    Proposition~\ref{prop:frobenius-hom} shows that \(\Phi_p\) is an automorphism
    so the \hyperref[prop:perfect-frobenius-bijective]{proposition above} applies.
\end{proof}
...
\end{proposition}
\begin{proof}
    The only algebraic extension of an algebraically closed field is itself,
    which is separable by definition.
\end{proof}
...
%     that \(K(a^p) \subsetneq K(a)\) is a strictly smaller field.
%     Then we can see that \(a \notin K(a^p)\),
%     because if it were so, then \(K(a) \subseteq K(a^p) \subsetneq K(a)\) is a contradicts equality.
% \end{proof}
\begin{theorem}\label{thm:perfect-field-condition}
    A field \(K\) is perfect
    if and only if its characteristic is 0,
    or if its characteristic is \(p\) and \(K^p = K\).
\end{theorem}
\begin{proof}
    There are three statements that we need to prove here:
    \begin{enumerate}[label={(\alph*)}, itemsep=0mm]
        \item if \(K\) has characteristic 0, then it is perfect;
        \item if \(K\) has characteristic \(p\) and \(K^p = K\), then it is perfect; and
        \item if \(K\) is perfect and has characteristic \(p\), then \(K^p = K\).
    \end{enumerate}
    Proposition~\ref{prop:perfect-frobenius-bijective} has already proven the last two statements,
    so it suffices to prove the first one.
    
    To prove this statement, it suffices to show that all irreducible polynomials are separable.
    But Proposition~\ref{prop:irreducible-multiplicity}a shows exactly that.
\end{proof}
...
\begin{theorem}\label{thm:normal-upper}
    Suppose \(F \subseteq K \subseteq L\) is a tower of extensions.
    If \(L/F\) is normal, then \(L/K\) is normal.
\end{theorem}
\begin{proof}
    Consider some \(\alpha \in L\), and let \(p(x) = \min_F(\alpha)\) and \(h(x) = \min_K(\alpha)\).
    Since we know that \(h(x) \mid p(x)\),
    so there exists some \(g \in K[x]\) such that \(h(x)g(x) = p(x)\).
    If \(p(x)\) splits completely, then \(h(x)\) should also split completely.
\end{proof}
...
%     % TODO
% \end{proof}
\begin{lemma}\label{lem:normal-intersection}
...
\end{lemma}
\begin{proof}
    Consider any irreducible \(f(x) \in F[x]\) that has roots in every \(E_i\).
    Since it splits completely in \(E_i\), the roots of \(f(x)\) reside in every \(E_i\),
    so it should also split completely in \(\bigcap E_i\).
\end{proof}
\begin{lemma}
...
\end{lemma}
\begin{proof}
    First suppose that \(F\) is perfect.
    Then every extension of \(F\) is separable, so \(K_\text{sep} = K\),
    and we are immediately done.

    Now suppose \(F\) is imperfect.
    By Theorem~\ref{thm:perfect-field-condition},
    we know that \(F\) has characteristic \(p\) and \(F^p \neq F\).
    Since we already know that \(K/K_\text{sep}\) is normal from Theorem~\ref{thm:normal-upper},
    it suffices to prove that \(K_\text{sep}/F\) is normal.
    Consider some \(\alpha \in K_\text{sep}\), and let \(p(x) = \min_F(\alpha)\).
    If \(p(x) = (x-\alpha)(x-r_2)\cdots(x-r_d)\) in a splitting field,
    we observe that by separability, the \(r_i\) are all distinct.
    Since each \(r_i\) also has the minimal polynomial \(\min_F(r_i) = p(x)\),
    we conclude that they all lie in \(K_\text{sep}\),
    so indeed \(K_\text{sep}/K\) is normal.
\end{proof}
...
\begin{proof}
    ...
    Let \(a \in E\), and \(p(x) = \min_F(a) \in F[x]\) its minimal polynomial.
    If \(b \in \overline{F}\) is a root of \(p(x)\),
    then there exists an \(F\)-isomorphism \(\func{\tau}{F(a)}{F(b)}\).
    But because \(\img\sigma = E\), we know that \(b \in E\),
    and in particular, all roots of \(p(x)\) have to be in \(E\).
    Hence \(E\) is the splitting field of \({\{\min_F(a)\}}_{a \in E}\) by construction.

    Now we prove that (ii) implies (iii).
    Suppose \(E\) is the splitting field of of a set of polynomials \({\{p_\alpha(x)\}}_{\alpha \in I}\).
    Let \(a \in E\), and \(f(x) = \min_F(a)\).
    Let \(b \in \overline{F}\) be any other root of \(f(x)\).
    Since there exists an \(F\)-isomorphism \(\func{\tau}{F(a)}{F(b)}\) such that \(\tau(a) = b\),
    we know that \(\tau\) extends to an \(F\)-embedding \(\func{\sigma}{E}{\overline{F}}\).
    As \(\sigma\) maps roots of \(p_\alpha(x)\) to roots of \(p_\alpha(x)\),
    we know that \(\img\sigma \subseteq E\).
    Using the \hyperref[thm:pigeonhole]{pigeonhole principle} on an \(F\)-basis of \(E\),
    we get \(\img\sigma = E\) and hence \(b \in E\), so \(f(x)\) does split into linear factors.

    Finally we prove that (iii) implies (i).
    Let \(a \in E\), with \(p(x) = \min_F(a)\) splitting completely over \(E\).
    But then \(\sigma(a)\) is just a root of \(p(x)\), so \(\img\sigma \subseteq E\).
    Same pigeonhole argument as the previous part applies, and we are done.
\end{proof}
...
\begin{proposition}
    ...
    \begin{enumerate}[label={(\alph*)}, itemsep=0mm]
        \item If \(K/F\) is normal, and \(\tau \in \Aut_F(L)\) is an \(F\)-automorphism,
            then \(\eval{\tau}_K \in \Aut_F(K)\).
        \item If \(L/F\) is normal, then any \(\sigma \in \Hom_F(K,L)\)
            extends to an \(F\)-automorphism over \(L\).
    \end{enumerate}
\end{proposition}
\begin{proof}
    Consider \(\tau \in \Aut_F(L)\), and consider it as a morphism \(\tau \in \Hom_F(L,\overline{F})\).
    Under a restriction, it becomes \(\eval{\tau}_K \Hom_F(K,\overline{F})\).
    But then from the \hyperref[thm:normal-extension-equivalence]{theorem above},
    \(\tau_K\) must be an automorphism.
    This proves part (a).

    Similarly, any \(\sigma \in \Hom_F(K,L)\) can be considered as \(\sigma \in \Hom_F(K,\overline{F})\),
    and then by Propostiion~\ref{prop:algebraic-extension-hom},
    we know that this extends to some homomorphism \(\tau \in \Hom_F(L,\overline{F})\).
    But then from the \hyperref[thm:normal-extension-equivalence]{theorem above},
    \(\tau\) must be an automorphism.
    This proves part (b).
\end{proof}
...
\begin{lemma}\label{lem:finite-mult-group-cyclic}
    ...
    and \(G \subseteq K^\times\) a finite subgroup of the multiplicative group.
    Then \(G\) is cyclic.
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{corollary}\label{cor:finite-field-simple}
    If \(K/F\) is an finite extension of finite fields,
    then \(K = F(\theta)\) is a simple extension.
\end{corollary}
\begin{proof}
    Consider \(\abs{K} = p^n\), so \(\abs{K^\times} = p^n - 1\).
    % If \(a \in K^\times\), then \(a^{p^{n-1}} = 1\).
    ...
    %
    % Conclude from the \hyperref[thm:primitive-element]{primitive element theorem}
    % and the \hyperref[lem:finite-mult-group-cyclic]{lemma above},
    Conclude from the \hyperref[lem:finite-mult-group-cyclic]{lemma above},
    since cyclic groups have a single generator.
\end{proof}
\begin{remark}
    In fact the primitive element does not have to be a generator of the multiplicative group.
\end{remark}
...
\begin{proposition}\label{prop:finite-field-size-inclusion}
...
\end{proposition}
\begin{proof}
    We just proved that \(\bF_{p^n}\) is the splitting field of \(x^{p^n} - x \in \bF_p[x]\).
    Consider some \(a \in \bF_{p^n}\), and let \(m = [\bF_p(a):\bF_p] \mid [\bF_{p^n}:\bF_p]\).
    As \(a\) is a root if \(x^{p^n} - x\), we have that \(\min_{\bF_p}(a) \mid x^{p^n} - x\).
    Hence all monic irreducible polynomials are factors of \(x^{p^n} - x\).

    Now, we wish to show that these are all the divisors.
    Consider any monic irreducible \(f \in \bF_p[x]\) of degree \(m \mid n\),
    and let \(K\) be the splitting field of \(f\) over \(\bF_p\).
    If \(a \in K\) is a root of \(f\), then \(K = \bF_p(a)\), so \([K:\bF_p] = m\),
    so \(K \subseteq \bF_{p^n}\) from the \hyperref[prop:finite-field-size-inclusion]{proposition above}.
    Hence \(a \in \bF_{p^n}\), and \(a\) is a a root if \(x^{p^n} - x\).
    As \(x^{p^n} - x\) has no repeated roots, we are done.
\end{proof}

\begin{lemma}\label{lem:extension-polynomial-divides}
...
\end{lemma}
\begin{proof}
    If \(f\) has a leading coefficient \(a\),
    then the minimal polynomial \(\min_{\bF_q}(\alpha)\)
    must be the corresponding monic polynomial \(f/a\).
    Then Proposition~\ref{prop:minimal-polynomial-factor} suffices.
\end{proof}
\begin{remark}
    This lemma actually holds more generally over arbitrary fields,
    but we don't quite need to use it here.
\end{remark}
\begin{lemma}\label{lem:factor-divisibility}
    Let \(f \in \bF_q[x]\) be an irreducible polynomial of degree \(m\).
    Then \(f \mid x^{q^n} - x\) if and only if \(m \mid n\).
\end{lemma}
\begin{proof}
    Suppose \(f \mid x^{q^n} - x\).
    Let \(\alpha\) be a root of \(f\) in some splitting field.
    Then \(\alpha^{q^n} = \alpha\), which means \(\bF_q(\alpha) \subseteq \bF_{q^n}\).
    As \([\bF_q(\alpha):\bF_q] = m\)
    and \(n = [\bF_{q^n}:\bF_q] = [\bF_{q^n}:\bF_q(\alpha)][\bF_q(\alpha):\bF_q] = [\bF_{q^n}:\bF_q(\alpha)]m\),
    we indeed have \(m \mid n\).

    For the reverse direction, suppose that \(m \mid n\),
    which impleis that \(\bF_{q^m} \subseteq \bF_{q^n}\).
    Consider \(\alpha\) as a root of \(f\) in some splitting field.
    Then \([\bF_q(\alpha):\bF_q] = m\), which gives us that \(\bF_q(\alpha) = \bF_{q^m}\).
    Thus \(\alpha \in \bF_{q^m}\), and \(\alpha^{q^n} = \alpha\),
    giving us that \(\alpha\) is a root of the polynomial \(x^{q^n} - x \in \bF_q[x]\).
    The \hyperref[lem:extension-polynomial-divides]{previous lemma} then sufficies.
\end{proof}
\begin{corollary}
...
\end{corollary}
\begin{proof}
    Counting argument from \hyperref[lem:factor-divisibility]{lemma above}.
\end{proof}
