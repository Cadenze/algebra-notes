...
\begin{definition}
    ...
    induces an arrow \(\tau X \in \Hom(SX,TX)\) for every object \(X \in \Ob(\mca{C})\).
    It is a `morphism of functors' in the sense that
    if \(X,Y \in \Ob(\mca{C})\) and \(f \in \Hom(X,Y)\),
    \begin{equation*}
        \begin{tikzcd}
            X \arrow{d}{f} & SX \arrow{d}{Sf} \arrow{r}{\tau X} & TX \arrow{d}{Tf} \\
            Y & SY \arrow{r}{\tau Y} & TY
        \end{tikzcd}
    \end{equation*}
    is commutative.
\end{definition}
\begin{definition}
    ...
    if every \(\tau X\) (for all objects \(X\)) is an invertible arrow.
    In that case, \(\func{\tau^{-1}}{T}{S}\) formed by \({(\tau X)}^{-1}\) is also a natural isomorphism.
\end{definition}
...
\begin{definition}
    An object \(X\) is an initial object of a category
    if there exists a morphism from \(X\) to any object \(Y\) in the category.
    An object \(X\) is a terminal object of a category
    if there exists a morphism from any object \(Y\) in the category to \(X\).
    An object \(0\) is the null object of a category
