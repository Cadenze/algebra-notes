...
\begin{proposition}
    ...
    and \(B \subset V\) forms a basis.
    The dual basis is \(B' \subset V'\),
    which for each \(\vec{u} \in B\),
    consists of elements \(\phi_{\vec{u}} \in B'\),
    where \(\phi_{\vec{u}}(\vec{v}) = \delta_{\vec{u},\vec{v}}\)
