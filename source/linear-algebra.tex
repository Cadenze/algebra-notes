...
\section{General Constructions}\label{sec:vsp-constructions}
...
\begin{proposition}
    Suppose \({\{V_i\}}_{i \in I},{\{W_i\}}_{i \in I}\) are two families of vector spaces,
    and \(\func{f_i}{V_i}{W_i}\) are homomorphisms connecting them.
    Then \(\vfunc{\bigoplus f_i}{\bigoplus V_i}{\bigoplus W_i}{\sum v_i}{\sum f_i(v_i)}\)
    is a vector space homomorphism.
\end{proposition}
\begin{proof}
    We can first observe from the following commutative diagram:
    \begin{equation*}
        \begin{tikzcd}
            V_i \arrow{r} \arrow{d}[swap]{f_i} \arrow{rd} &
            \bigoplus V_i \arrow[dashrightarrow]{d}{\exists!} \\
            W_i \arrow{r} & \bigoplus W_i
        \end{tikzcd}
    \end{equation*}
    Considering some \(\sum v_i \in \bigoplus V_i\),
    there are corresponding \(v_i \in V_i\) for each \(i\) embedded into it.
    Then immediately we see that we have \(f_i(v_i) \in W_i\),
    and we can embed all of those to form \(\sum f_i(v_i) \in \bigoplus W_i\),
    which means our dashed homomorphism is our desired one.
\end{proof}
...
\begin{proposition}
    Suppose \({\{V_i\}}_{i \in I},{\{W_i\}}_{i \in I}\) are two families of vector spaces,
    and \(\func{f_i}{V_i}{W_i}\) are homomorphisms connecting them.
    Then \(\vfunc{\prod f_i}{\prod V_i}{\prod W_i}{(v_i)}{\prod (f_i(v_i))}\)
    is a vector space homomorphism.
\end{proposition}
\begin{proof}
    One can trace through the commutative diagram:
    \begin{equation*}
        \begin{tikzcd}
            V_i \arrow{d}[swap]{f_i} &
            \prod V_i \arrow{l} \arrow{ld} \arrow[dashrightarrow]{d}{\exists!} \\
            W_i & \prod W_i \arrow{l}
        \end{tikzcd}
    \end{equation*}
    and see that if we have a product \((v_i) \in \prod V_i\),
    then this projects onto each \(v_i \in V_i\),
    mapping them to \(f_i(v_i) \in W_i\).
    But this can exactly be projected by \((f_i(v_i)) \in \prod W_i\) as desired.
\end{proof}
...
\begin{proposition}[Universal Property of Quotients]\label{prop:univ-prop-quotient-vsp}
    ...
    \begin{center}
        \begin{tikzcd}
            V \arrow{d}{\pi} \arrow{r}{\phi} & Z \\
            V/U \arrow[dashrightarrow]{ru}[swap]{\exists! \bar{\phi}}
        \end{tikzcd}
    \end{center}
\end{proposition}
\begin{proof}
    ...
    \end{equation*}
\end{proof}
\begin{theorem}[Uniqueness of Quotients]
...
\begin{proof}
    ...
    \begin{center}
        \begin{tikzcd}
            V \arrow{r}{\pi'} \arrow{d}{\pi} &
            W \arrow[rightharpoondown, shift right=0.25ex]{ld}%
                [xshift=.5ex, yshift=-.5ex, swap]{\bar{\phi}'} \\
            V/U \arrow[rightharpoondown, shift right=0.25ex]{ru}%
                [xshift=-.5ex, yshift=.5ex, swap]{\bar{\phi}}
        \end{tikzcd}
    \end{center}
...
\begin{remark}
    ...
    \hyperref[thm:iso-1-mod]{isomorphism theorems on modules},
...
\begin{remark}
    ...
    in Proposition~\ref{prop:module-hom-functor} for modules over commutative rings.
\end{remark}
% \begin{proposition}
...
\begin{theorem}[Uniqueness of Tensor Product]\label{thm:tensor-unique}
...
\begin{theorem}[Existence of Tensor Product]\label{thm:tensor-exists}
    Suppose \({\{V_i\}}_{i \in I}\) is a family of vector spaces,
...
\begin{proof}
    ...
    % If \(\vec{v}\phi = \vec{w}\psi\),
    % then \(\vec{v}\sum a_i\phi_i = \vec{w}\sum b_i\phi_i\).
    % Hence for \(\vec{u}_i\) in the basis,
    % \begin{equation*}
    %     a_i\vec{v} = \vec{v}\sum a_i\phi_i(\vec{u}_i)
    %     = \vec{w}\sum b_i\phi_i(\vec{u}_i) = b_i\vec{w}
    % \end{equation*}
    % If \(a_i = b_i\) for all \(i\), then \(\phi = \psi\),
    % and the equality above gives \(\vec{v} = \vec{w}\), and we are done.
    % If \(a_i \neq b_i\) for some \(i\), then \(\vec{v} = \frac{b_i}{a_i}\vec{w} = c\vec{w}\),
    % and in fact \(c = \frac{b_1}{a_1} = \cdots = \frac{b_n}{a_n}\) as a result;
    % then \(\psi = c\phi\), which gives us
    % \(\vec{v}\otimes\phi = c\vec{w}\otimes\phi = \vec{w}\otimes\psi\) as desired.
    % Proving this for pure tensors is sufficient as all tensors are linear combinations of pure ones.
    Let us prove surjectivity.
    % Consider \(\vec{u} = \sum a_i\vec{u}_i\).
    ...
    % Then let us choose bases \({\{\vec{v}_i\}}_{i=1}^n \subset V\),
    % \({\{\vec{u}_j\}}_{j \in I} \subset U\),
    % \({\{\phi_j\}}_{j \in \tilde{I}} \subset U'\),
    % where \(I \subset \tilde{I}\),
    % i.e.\ the basis of \(U'\) is extended from the dual basis.
    % We prove that this map is injective.
    % If \(\vec{v}\phi = \vec{w}\psi\),
    % then \(\sum a_i\vec{v}_i\phi = \sum b_i\vec{v}_i\psi\).
    % From the previous paragraph, if \(\phi = c\psi\), then we are done.
    % Now suppose \(\phi \neq \psi\).
    % Then there exists some \(\vec{u} \in U\) such that \(\phi(\vec{u}) \neq \psi(\vec{u})\),
    % and in that case, let \(\phi(\vec{u}) = c\psi(\vec{u})\).
    % We have \(\vec{w} = c\vec{v}\), so \(\phi = c\psi\), which reduces to the first case.
    Let us prove surjectivity.
    % We know that any map in \(\Hom(U,V)\)
    % is wholly determined by the image of its basis,
    % which we let to be \(\vec{u}_i\mapsto\sum_j a_j\vec{v}_j\).
    % This is mapped from \(\sum_j a\vec{v}_j\otimes\vec{u}_i\), since
    % \begin{equation*}
    %     f\pqty{\sum_{j=1}^n a_j\vec{v}_j\otimes\vec{u}_i}
    % \end{equation*}
...
\begin{proof}
    ...
    % Suppose \({\{1\otimes\vec{v}_i\}}_{i \in I} \subset V_K\) is spanning.
    % Then any vector \(\vec{w} = \sum \alpha_i(1\otimes\vec{v}_i) = \sum a_{ij} \beta_j \otimes \vec{v}_i\),
    % using the basis \({\{\beta_j\}}_{j \in J} \subset K\).
...
\begin{proof}
    ...
    \begin{center}
        \begin{tikzcd}
            U_K \arrow{r}{\phi} \arrow{d}{\pi_K} & Z \\
            \pqty{U/V}_K \arrow[dashrightarrow]{ru}[swap]{\bar{\phi}}
        \end{tikzcd}
    \end{center}
    ...
    \begin{center}
        \begin{tikzcd}
            U_K \times V_K \arrow{r}{\phi} \arrow{d}{\iota} & X \\
            \pqty{U \otimes V}_K \arrow[dashrightarrow]{ru}[swap]{\bar{\phi}}
        \end{tikzcd}
    \end{center}
...
\begin{lemma}\label{lem:det-direct-sum}
...
\begin{lemma}\label{lem:eigenvalue-min-poly}
...
\begin{proposition}\label{prop:min-poly-direct-sum}
...
\begin{lemma}\label{lem:eigenspace-direct-sum}
    Given some \(T \in \End(V)\),
...
\begin{lemma}\label{lem:nilpotent-degree}
...
\end{proof}
\begin{theorem}[Cayley-Hamilton Theorem]\label{thm:cayley-hamilton}
    Suppose \(\dim V = n\).
...
\section{Jordan Canonical Form}\label{sec:jcf}
...
\begin{theorem}\label{thm:jordan-canonical-nilpotent}
...
\begin{theorem}[Existence of Jordan Canonical Form]\label{thm:jordan-canonical-form}
    Suppose \(T \in \End(V)\),
...
\end{remark}
\begin{theorem}[Uniqueness of Jordan Canonical Form]
